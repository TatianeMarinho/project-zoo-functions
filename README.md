...
Funções que buscam informações sobre os animais do zoológico como: espécie e local de origem. Além disso, também busquei dados sobre as pessoas que colaboram com a manutenção e cuidado do zoológico. E foi implementado testes para funções já existentes, percebendo os casos de uso da sua aplicação e garantindo que ela está funcionando da maneira correta! 
...
Nesse projeto implementei testes para duas funções já criadas: handlerElephants e getOpeningHours.
...
A pasta `src` é composta por arquivos referentes a cada uma das funções que eu implementei, a pasta `data` contém o arquivo `zoo_data.js`, que armazena os dados que utilizado para retornar as informações sobre o zoológico, feito pela Trybe.

Este projeto possui também alguns requisitos para a implementação de testes. A pasta `test`, contém os arquivos que eu implementarei os testes unitários correspondentes a cada função, o nome do arquivo é o nome da função alvo acrescido do sufixo `.test.js`.

**Por exemplo:** o arquivo `src/getOpeningHours.js` conterá a implementação da função `getOpeningHours` e o arquivo `test/getOpeningHours.test.js` deve conter os testes que eu desenvolvi referentes a ela.

A pasta `evaluator` possui os testes automatizados que avaliarão o seu projeto; os conteúdos desta pasta foram criados pela Trybe.
